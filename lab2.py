...
def validate_password(password):
    if len(password) < 8:  # Минимум 8 символов
        return False
    if not re.search(r'[A-Z]', password):  # Есть заглавная буква
        return False
    if not re.search(r'[a-z]', password):  # Есть строчная буква
        return False
    if not re.search(r'[0-9]', password):  # Есть цифра
        return False
    if not re.search(r'[!@#$%^&*]', password):  # Есть спецсимвол
        return False
    return True
...
def normalize_phone(phone):
    ...
    if len(digits) == 11:
        return f"+7{digits[1:]}"  # +7 и остальные 10 цифр
    elif len(digits) == 10:
        return f"+7{digits}"  # Добавляем +7
    else:
        return None
